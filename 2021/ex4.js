...
const calc = (arr) => {
    ...
    for (let i = 0; i < callNumbers.length; i++) {
        ...
        if (winner) {
            calcWinner(cleanedBoards[winner], callNumbers[i])
            break;
        }
    }
...
const calcWinner = (table, calledNum) => {
    ...
    table.forEach(row => {
        row.forEach(number => {
             if (!number.marked) num += parseInt(number.num)
        })
    })

    console.log(num * parseInt(calledNum))

    return num * parseInt(calledNum)
}
