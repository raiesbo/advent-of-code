...
fs.readFile(__dirname + fileUrl, (error, data) => {
    ...
    calc(cleanData) // Result: 5698
});

const calc = arr => {
    const cleanedCoords = cleanCoords(arr);
    const purgedCoords = discardCoords(cleanedCoords);
    let diagram = createDiagram(cleanedCoords);

    for (let coordsLine of purgedCoords) {
...
const discardCoords = arr => {
    return arr.filter(coords => coords.x1 === coords.x2 || coords.y1 === coords.y2)
}
