...
// 4 shiny gold (n , nom)
const resolveBags = (bag) => {
    let trimmed = bag.trim()
    let n = parseInt(trimmed[0]); 
    let formatted = trimmed.slice(2).replace(".", "").replace("bags", "").replace("bag", "");
    return [n, formatted]
}
...
const bagChecker = (arr) => {
    ...
    for (let bag of arr) {
        ...
        let left = subBag[0]
        let right = subBag[1].split(", ")
        
        for(let bagRight of right) {
            let resolved = resolveBags(bagRight)
            let n = resolved[0];
            let b = resolved[1];
            if (left.includes("shiny gold") && !shinyGoldBagContainters.includes(left.slice(0, -2))) {
                shinyGoldBagContainters.push(left.slice(0, -2))
            }
        }
    }
    ...
    for (let i = 0 ; i < 10; i ++) {
        for (let bag of arr) {
            ...
            for (let color of shinyGoldBagContainters) {
                if (subBag[1].includes(color) 
                    && !shinyGoldBagContainters.includes(subBag[0].slice(0, -2)) 
                    && !shinyGoldBagContainters.includes(subBag[0].slice(0, -2))
                ) {
